...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(list_points)):
        point = list_points[k]
        mini_circle = rg.Circle(point,10)
        mini_circle.fill_color = color
        mini_circle.attach_to(window)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(list_points)):
        line = rg.Line(list_points[k-1], list_points[k])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 3:  16 black lines on magenta circle, ' \
            'hops = 7.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 300), 150)
    circle.fill_color = 'magenta'
    fancy_polygon(window, circle, 16, 7, 'black', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(list_points)):
        start_index = k
        end_index = k + hops_to_next_point
        if end_index >= len(list_points):
            end_index = end_index %  number_of_lines
        line = rg.Line(list_points[start_index], list_points[end_index])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
